...
const api = {
  ...
  admin: {
    ...
    getFollowupConfig: () => request('/admin/followup/config'),
    updateFollowupConfig: (days) => request('/admin/followup/config', { method: 'POST', body: JSON.stringify({ reminder_advance_days: days }) }),
    getFollowups: (params = {}) => {
      const qs = new URLSearchParams(params).toString();
      return request(`/admin/followup${qs ? `?${qs}` : ''}`);
    },
    getFollowup: (id) => request(`/admin/followup/${id}`),
    cancelFollowup: (id, reason) => request(`/admin/followup/${id}/cancel`, { method: 'POST', body: JSON.stringify({ cancel_reason: reason }) }),
    exportFollowupCSV: (params = {}) => {
      const qs = new URLSearchParams(params).toString();
      return `/api/admin/followup/export${qs ? `?${qs}` : ''}`;
    },
  ...
  nurse: {
    ...
    getTodayFollowups: (deptId) => request(`/nurse/followup/today${deptId ? `?department_id=${deptId}` : ''}`),
    getFollowups: (params = {}) => {
      const qs = new URLSearchParams(params).toString();
      return request(`/nurse/followup${qs ? `?${qs}` : ''}`);
    },
    getFollowup: (id) => request(`/nurse/followup/${id}`),
    recordFollowupContact: (id, status, result) => request(`/nurse/followup/${id}/contact`, { method: 'POST', body: JSON.stringify({ status, contact_result: result }) }),
  ...
  doctor: {
    ...
    createFollowup: (data) => request('/doctor/followup', { method: 'POST', body: JSON.stringify(data) }),
    getFollowups: (params = {}) => {
      const qs = new URLSearchParams(params).toString();
      return request(`/doctor/followup${qs ? `?${qs}` : ''}`);
    },
    getFollowup: (id) => request(`/doctor/followup/${id}`),
    cancelFollowup: (id, reason) => request(`/doctor/followup/${id}/cancel`, { method: 'POST', body: JSON.stringify({ cancel_reason: reason }) }),
  },
